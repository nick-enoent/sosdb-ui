...
(function ($) {
    ...
    $.fn.rowGrouping = function (options) {
		...
		var nAccord = document.createElement('td');
                ...
                if (properties.bSetGroupingClassOnTR) {
                    ...
                    nAccord.className = properties.sGroupingClass + " " + sGroupCleaned;
                }
                ...
                if (properties.bExpandableGrouping) {
                    ...
                    if (!_fnIsGroupCollapsed(sGroupCleaned)) {
                        nAccord.className += " expanded-group";
                        oGroup.state = "expanded";
                    } else {
                        nAccord.className += " collapsed-group";
                        oGroup.state = "collapsed";
                    }
                    nAccord.className += " group-item-expander";
                    $(nAccord).attr('data-group', oGroup.dataGroup); //Fix provided by mssskhalsa (Issue 5)
                    $(nAccord).attr("data-group-level", oGroup.level);
                    $(nAccord).click(_fnOnGroupClick);
                }
		nGroup.appendChild(nAccord);
